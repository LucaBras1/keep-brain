...
import { keepConnectSchema, keepConnectPasswordSchema, keepConnectTokenSchema, getZodErrorMessage } from "@/lib/validations"
...
export async function POST(request: Request) {
  try {
    ...
    const isTokenAuth = 'masterToken' in body

    if (!isPasswordAuth && !isOAuthAuth && !isTokenAuth) {
      return NextResponse.json(
        { error: "Musíte zadat OAuth token, Master token nebo App Password" },
        { status: 400 }
      )
    }

    if (isTokenAuth) {
      // Direct master token authentication
      const result = keepConnectTokenSchema.safeParse(body)
      ...
      const { email, masterToken } = result.data
